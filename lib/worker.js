...
process.on('message', function (msg) {
  switch (msg.type) {
    case 'die':
      console.error('worker with pid ' + process.pid + ' received die message');
      setTimeout(function () {
        console.error('worker with pid ' + process.pid + ' about to exit');
        process.exit(0);
        console.error('worker with pid ' + process.pid + ' went right by the exit');
      }, msg.time);
      console.error('worker with pid ' + process.pid + ' set timeout to die');
      break;
    case 'ready':
      ready();
      break;
